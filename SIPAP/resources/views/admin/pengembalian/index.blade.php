        ...
        <div class="border-b border-gray-200">
            <nav class="-mb-px flex space-x-6" aria-label="Tabs">
                <button @click="activeTab = 'aktif'" :class="activeTab === 'aktif' ? 'border-[#0EA5E9] text-[#0EA5E9] font-semibold border-b-2' : 'border-transparent text-[#94A3B8] hover:text-[#334155]'" class="pb-4 px-1 text-sm font-medium transition cursor-pointer whitespace-nowrap">
                    ...
                    <span class="ml-1.5 bg-[#0EA5E9]/10 text-[#0EA5E9] text-[10px] font-bold px-1.5 py-0.5 rounded-full">{{ $peminjamans->count() }}</span>
                </button>
                <button @click="activeTab = 'riwayat'" :class="activeTab === 'riwayat' ? 'border-[#0EA5E9] text-[#0EA5E9] font-semibold border-b-2' : 'border-transparent text-[#94A3B8] hover:text-[#334155]'" class="pb-4 px-1 text-sm font-medium transition cursor-pointer whitespace-nowrap">
                    ...
                    <span class="ml-1.5 bg-slate-100 text-slate-500 text-[10px] font-bold px-1.5 py-0.5 rounded-full">{{ $pengembalians->count() }}</span>
                </button>
            </nav>
        </div>
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-100 text-left text-sm">
                        ...
                        <tbody class="divide-y divide-gray-100 bg-white text-[#334155]">
                             ...
                             @endphp
                             <tr class="hover:bg-slate-50/50 transition {{ $isLate ? 'bg-red-50/30' : '' }}">
                                 ...
                                 <td class="px-6 py-4 whitespace-nowrap font-medium">{{ $item->alat->nama_alat ?? 'N/A' }}</td>
                                 <td class="px-6 py-4 text-slate-500 whitespace-nowrap">{{ $item->tgl_pinjam->format('d M Y') }}</td>
                                 <td class="px-6 py-4 whitespace-nowrap">
                                     <span class="{{ $isLate ? 'text-red-600 font-semibold' : 'text-slate-500' }}">
                                 ...
                                 <td class="px-6 py-4 text-right whitespace-nowrap">
                                     <button type="button"
                                         ...
                                         @click="showModal = true; selectedPeminjamanId = '{{ $item->id }}'; selectedAlatName = '{{ $item->alat->nama_alat ?? '' }}'">
                                         Verifikasi Kembali
                                     </button>
                                 </td>
                                 @endif
                    ...
                    <table class="min-w-full divide-y divide-gray-100 text-left text-sm">
                        ...
                        <tbody class="divide-y divide-gray-100 bg-white text-[#334155]">
                            ...
                            <tr class="hover:bg-slate-50/50 transition">
                                <td class="px-6 py-4 text-slate-400 font-medium">{{ $index + 1 }}</td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="font-medium">{{ $p->peminjaman->user->name ?? 'N/A' }}</div>
                                    <div class="text-[10px] text-slate-400">{{ $p->peminjaman->user->email ?? '' }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap font-medium">{{ $p->peminjaman->alat->nama_alat ?? 'N/A' }}</td>
                                <td class="px-6 py-4 text-slate-500 whitespace-nowrap">{{ \Carbon\Carbon::parse($p->tgl_kembali_aktual)->format('d M Y, H:i') }}</td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    @if(str_contains(strtolower($p->kondisi_alat ?? ''), 'rusak'))
