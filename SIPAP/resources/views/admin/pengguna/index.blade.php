    ...
    <div class="space-y-6">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-4 flex items-center gap-4">
                ...
                <div>
                    <p class="text-2xl font-bold text-[#0369A1]">{{ $adminCount }}</p>
            ...
            <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-4 flex items-center gap-4">
                ...
                <div>
                    <p class="text-2xl font-bold text-sky-600">{{ $dosenCount }}</p>
            ...
            <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-4 flex items-center gap-4">
                ...
                <div>
                    <p class="text-2xl font-bold text-purple-600">{{ $mahasiswaCount }}</p>
                ...
                <table class="min-w-full divide-y divide-gray-100 text-left text-sm">
                    ...
                    <tbody class="divide-y divide-gray-100 bg-white text-[#334155]">
                        ...
                        <tr class="hover:bg-slate-50/50 transition">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center gap-3">
                                    <div class="w-9 h-9 rounded-full flex items-center justify-center text-white font-bold text-sm flex-shrink-0
                                        {{ $user->role === 'admin' ? 'bg-[#0369A1]' : ($user->role === 'dosen' ? 'bg-sky-500' : 'bg-purple-500') }}">
                                        {{ substr($user->name, 0, 1) }}
                                    </div>
                                    <div>
                                        <p class="font-semibold text-[#334155]">{{ $user->name }}</p>
                                    </div>
                                </div>
                            </td>
                            ...
                            <td class="px-6 py-4 text-slate-500 font-mono text-xs">{{ $user->identifier ?? '-' }}</td>
