...
@section('content')
    <div class="max-w-4xl mx-auto space-y-6">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6" x-data="{ isSaving: false }">
            <form action="{{ route('alat.store') }}" method="POST" enctype="multipart/form-data" @submit="isSaving = true">
                @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <input type="text" name="nama_alat" value="{{ old('nama_alat') }}" required placeholder="Contoh: Proyektor Epson EB-X05" class="border border-gray-200 rounded-lg px-3.5 py-2 text-sm w-full focus:outline-none focus:ring-2 focus:ring-[#38BDF8] focus:border-[#0EA5E9] transition @error('nama_alat') border-red-500 @enderror">
                            @error('nama_alat')
                        ...
                        <div>
                            ...
                            <input type="text" name="merk" value="{{ old('merk') }}" placeholder="Contoh: Epson" class="border border-gray-200 rounded-lg px-3.5 py-2 text-sm w-full focus:outline-none focus:ring-2 focus:ring-[#38BDF8] focus:border-[#0EA5E9] transition @error('merk') border-red-500 @enderror">
                            @error('merk')
                                <p class="text-xs text-red-500 mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <input type="number" name="stok" value="{{ old('stok', 1) }}" min="0" required class="border border-gray-200 rounded-lg px-3.5 py-2 text-sm w-full focus:outline-none focus:ring-2 focus:ring-[#38BDF8] focus:border-[#0EA5E9] transition @error('stok') border-red-500 @enderror">
                            @error('stok')
                                <p class="text-xs text-red-500 mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <textarea name="deskripsi" rows="4" placeholder="Detail spesifikasi alat..." class="border border-gray-200 rounded-lg px-3.5 py-2 text-sm w-full focus:outline-none focus:ring-2 focus:ring-[#38BDF8] focus:border-[#0EA5E9] transition @error('deskripsi') border-red-500 @enderror">{{ old('deskripsi') }}</textarea>
                            @error('deskripsi')
                                <p class="text-xs text-red-500 mt-1">{{ $message }}</p>
                            @enderror
                        </div>
