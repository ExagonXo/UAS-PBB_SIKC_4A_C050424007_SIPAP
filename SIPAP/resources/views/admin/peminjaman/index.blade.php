            ...
            <nav class="-mb-px flex space-x-6 overflow-x-auto" aria-label="Tabs">
                ...
                <button @click="activeTab = 'pending'" :class="activeTab === 'pending' ? 'border-[#0EA5E9] text-[#0EA5E9] font-semibold border-b-2' : 'border-transparent text-[#94A3B8] hover:text-[#334155]'" class="pb-4 px-1 text-sm font-medium transition cursor-pointer whitespace-nowrap flex items-center gap-1.5">
                    ...
                    <span class="bg-[#EF4444] text-white text-[10px] px-1.5 py-0.5 rounded-full font-bold">
                        {{ $peminjamans->where('status', 'pending')->count() }}
                    </span>
                </button>
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-100 text-left text-sm">
                    ...
                    <tbody class="divide-y divide-gray-100 bg-white text-[#334155]">
                        ...
                        <tr x-show="activeTab === 'semua' || activeTab === '{{ $displayStatus }}'" class="hover:bg-slate-50/50 transition">
                            <td class="px-6 py-4 text-slate-400 font-medium">{{ $no++ }}</td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap font-medium text-slate-700">
                                {{ $item->alat->nama_alat ?? 'None' }}
                            </td>
                            <td class="px-6 py-4 text-slate-500 whitespace-nowrap">{{ $item->tgl_pinjam->format('d M Y') }}</td>
                            <td class="px-6 py-4 text-slate-500 whitespace-nowrap">{{ $item->tgl_kembali_rencana->format('d M Y') }}</td>
                            <td class="px-6 py-4 whitespace-nowrap">
                            ...
                            <td class="px-6 py-4 text-right space-x-1 whitespace-nowrap">
                                <!-- Detail Button -->
                                <a href="{{ route('peminjaman.show', $item->id) }}" class="text-[#94A3B8] hover:text-[#334155] p-1.5 hover:bg-slate-100 rounded-lg transition inline-flex items-center" title="Detail">
                                ...
                                @if(Auth::user()->role === 'admin')
                                    ...
                                    @if($item->status === 'pending')
                                        <div class="inline-flex items-center gap-1">
                                            <form action="{{ route('peminjaman.approve', $item->id) }}" method="POST" class="inline-block">
                                                @csrf
                                                @method('PATCH')
                                            ...
                                            <form action="{{ route('peminjaman.reject', $item->id) }}" method="POST" class="inline-block">
                                                @csrf
                                                @method('PATCH')
                                    ...
                                    @if($item->status === 'disetujui' && (!$item->alat || $item->alat->status !== 'dipinjam'))
                                        <form action="{{ route('peminjaman.handover', $item->id) }}" method="POST" class="inline-block">
                                            @csrf
                                            @method('PATCH')
