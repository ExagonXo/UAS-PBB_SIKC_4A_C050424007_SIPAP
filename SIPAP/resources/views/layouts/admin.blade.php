...
<head>
    ...
    <title>@yield('page-title') | SIPAP Admin</title>
            ...
            <div>
                ...
                <nav class="p-4 space-y-1">
                    ...
                    <a href="{{ route('dashboard') }}" 
                       class="group flex items-center py-2.5 px-4 text-sm transition-all duration-200 {{ $dashActive ? 'bg-[#0EA5E9]/20 text-white border-l-4 border-[#38BDF8] rounded-r-lg font-medium' : 'text-slate-300 hover:bg-white/10 hover:text-white rounded-lg' }}">
                        <svg class="w-5 h-5 mr-3 {{ $dashActive ? 'text-[#38BDF8]' : 'text-slate-400 group-hover:text-white' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <!-- Alat -->
                    @php $alatActive = request()->routeIs('alat.*'); @endphp
                    <a href="{{ route('alat.index') }}" 
                       class="group flex items-center py-2.5 px-4 text-sm transition-all duration-200 {{ $alatActive ? 'bg-[#0EA5E9]/20 text-white border-l-4 border-[#38BDF8] rounded-r-lg font-medium' : 'text-slate-300 hover:bg-white/10 hover:text-white rounded-lg' }}">
                        <svg class="w-5 h-5 mr-3 {{ $alatActive ? 'text-[#38BDF8]' : 'text-slate-400 group-hover:text-white' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <!-- Peminjaman -->
                    @php $pemActive = request()->routeIs('peminjaman.*'); @endphp
                    <a href="{{ route('peminjaman.index') }}" 
                       class="group flex items-center justify-between py-2.5 px-4 text-sm transition-all duration-200 {{ $pemActive ? 'bg-[#0EA5E9]/20 text-white border-l-4 border-[#38BDF8] rounded-r-lg font-medium' : 'text-slate-300 hover:bg-white/10 hover:text-white rounded-lg' }}">
                        <div class="flex items-center">
                            <svg class="w-5 h-5 mr-3 {{ $pemActive ? 'text-[#38BDF8]' : 'text-slate-400 group-hover:text-white' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <!-- Pengembalian -->
                    @php $kemActive = request()->routeIs('pengembalian.*'); @endphp
                    <a href="{{ route('pengembalian.index') }}" 
                       class="group flex items-center py-2.5 px-4 text-sm transition-all duration-200 {{ $kemActive ? 'bg-[#0EA5E9]/20 text-white border-l-4 border-[#38BDF8] rounded-r-lg font-medium' : 'text-slate-300 hover:bg-white/10 hover:text-white rounded-lg' }}">
                        <svg class="w-5 h-5 mr-3 {{ $kemActive ? 'text-[#38BDF8]' : 'text-slate-400 group-hover:text-white' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    @php $userActive = request()->is('admin/pengguna*'); @endphp
                    <a href="/admin/pengguna" 
                       class="group flex items-center py-2.5 px-4 text-sm transition-all duration-200 {{ $userActive ? 'bg-[#0EA5E9]/20 text-white border-l-4 border-[#38BDF8] rounded-r-lg font-medium' : 'text-slate-300 hover:bg-white/10 hover:text-white rounded-lg' }}">
                        <svg class="w-5 h-5 mr-3 {{ $userActive ? 'text-[#38BDF8]' : 'text-slate-400 group-hover:text-white' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <!-- Log Notifikasi -->
                    @php $notifActive = request()->is('admin/notifikasi*'); @endphp
                    <a href="/admin/notifikasi" 
                       class="group flex items-center py-2.5 px-4 text-sm transition-all duration-200 {{ $notifActive ? 'bg-[#0EA5E9]/20 text-white border-l-4 border-[#38BDF8] rounded-r-lg font-medium' : 'text-slate-300 hover:bg-white/10 hover:text-white rounded-lg' }}">
                        <svg class="w-5 h-5 mr-3 {{ $notifActive ? 'text-[#38BDF8]' : 'text-slate-400 group-hover:text-white' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="flex items-center justify-between gap-3 p-2 bg-black/10 rounded-xl">
                    <div class="flex items-center gap-2.5 min-w-0">
                        ...
                        <div class="min-w-0">
                            ...
                            <p class="truncate text-[10px] text-slate-300 uppercase font-medium mt-0.5">{{ auth()->user()->role }}</p>
                        </div>
                    </div>
        ...
        <div class="flex-1 flex flex-col pl-[240px]">
            ...
            <header class="h-[60px] bg-white border-b border-[#F1F5F9] shadow-sm flex items-center justify-between px-6 sticky top-0 z-40">
                ...
                <div class="flex items-center gap-4">
                    <!-- Notification Bell -->
                    @php $unreadNotifs = \App\Models\Notifikasi::where('is_read', false)->count(); @endphp
                    ...
                    <div class="relative" x-data="{ open: false }" @click.outside="open = false">
                        ...
                        <button @click="open = !open" class="flex items-center gap-2.5 rounded-xl px-2.5 py-1.5 hover:bg-slate-50 transition cursor-pointer select-none">
                            <div class="hidden sm:block text-right">
                                <p class="text-xs font-semibold text-[#334155]">{{ auth()->user()->name }}</p>
                                <p class="text-[10px] text-[#94A3B8] capitalize">{{ auth()->user()->role }}</p>
                            </div>
                        ...
                        >
                            ...
                            <div class="px-4 py-3 bg-[#F8FAFC] border-b border-slate-100">
                                <p class="text-xs font-semibold text-[#334155] truncate">{{ auth()->user()->name }}</p>
                                <p class="text-[10px] text-[#94A3B8] truncate mt-0.5">{{ auth()->user()->email }}</p>
                            ...
                            <!-- Menu Items -->
                            <div class="p-1.5">
                                <a href="{{ route('profile.edit') }}" class="flex items-center gap-3 px-3 py-2 rounded-lg text-sm text-[#334155] hover:bg-slate-50 transition group">
